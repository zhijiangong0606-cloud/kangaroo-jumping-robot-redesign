...
def main():
    ...
    d.text((48, 28), "袋鼠仿生弹跳机器人 - SolidWorks 基础装配效果图", font=font(36, True), fill="white")
    d.text((50, 72), "电机预紧 + 弹性肌腱 + 锁止释放 + 可调尾部稳定", font=font(20), fill=(203, 213, 225))

    def iso(x, y, z):
        sx = 570 + x * 2.05 + y * 1.2
        sy = 410 + z * 1.75 - y * 0.65
        return sx, sy
    ...
    d.rounded_rectangle([1230, 180, 1715, 640], radius=16, fill="white", outline=(180, 190, 205), width=2)
    d.text((1260, 215), "装配模块", font=font(30, True), fill=ink)
    items = [
        ("机身双侧板", plate),
        ("一自由度闭链后肢", green),
        ("电机 + 绕线轮预紧", blue),
    ...
    d.text((125, 1045), "关键尺寸：H0-H1=100mm，H0-H2=134mm，L1=40mm，L2=120mm，L3=100mm，L4=140mm，L5=180mm，尾杆=180-220mm 可调", font=font(23, True), fill=ink)
    d.text((125, 1090), "说明：这是基础装配效果图，零件已生成 SolidWorks SLDPRT；后续可继续添加轴承座、减重孔、圆角和运动配合。", font=font(20), fill=muted)

    out = OUT / "final_assembled_render_cn.png"
